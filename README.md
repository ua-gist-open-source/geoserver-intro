...
Open QGIS. QGIS has been programmed to be able to parse this `getCapabilities` request to know what layers are available to issue requests for. 
...
Next we are going to load this WFS via QGIS in a fashion similar to what we did for WMS. In the Layers Toolbar, find the “Add WFS Layer” option, which has a vector icon, but is embedded in a graticuled map.

![add_wfs_layer.png](add_wfs_layer.png)
...
Q8: What is the URL of your coarse resolution sample of a WMTS url? What level does this tile refer to?
Notice the differences. What are some of the fields that are unique to this url?
